...
class downsample(nn.Module):
    ...
    def __init__(self, nbase, sz, conv_3D=False, max_pool=True):
        ...
        if max_pool:
            self.maxpool = (nn.MaxPool3d(kernel_size=(1,2,2)) if conv_3D
                            else nn.MaxPool2d(2, stride=2))
        else:
            self.maxpool = nn.AvgPool3d(kernel_size=(1,2,2)) if conv_3D else nn.AvgPool2d(2, stride=2)
...
class upsample(nn.Module):
    ...
    def __init__(self, nbase, sz, conv_3D=False):
        super().__init__()
        scale_factor = (1,2,2) if conv_3D else (2,2)
...
class CPnet(nn.Module):
    ...
    Args:
        ...
        sz (int): Kernel size. #not size of the input image
    ...
    def load_model(self, filename, device=None):
        ...
        if (device is not None) and (device.type != "cpu"):
            ...
            self.__init__(self.nbase, self.nout, self.sz, self.conv_3D,
                          self.diam_mean)
        ...
        if state_dict["output.2.weight"].shape[0] != self.nout:
            ...
            self.load_state_dict(
                ...
                strict=False)
def test():
    x = torch.randn((1, 1, 32, 32, 32)) #n_img, n_chan, z,x,y #must be divisible by 8 in this implementation
    model = CPnet(nbase=[1,32,64, 128, 256], nout=1, sz=3, mkldnn=False, conv_3D=True, max_pool=True,
                 diam_mean=30.)
    print(model)
